...
install_if_missing zsh tmux fzf ripgrep
...
# Install ASDF (Go version v0.16.0) if not already present
if ! command_exists asdf; then
  echo "Installing ASDF (Go version v0.16.0)..."
  # Clone the main repository using the v0.16.0 tag as per the guide
  ...
  # Add ASDF sourcing to Zshrc if not already present.
  # asdf.bash provides completions for Zsh too.
  if ! grep -q ". \"\$HOME/.asdf/asdf.sh\"" ~/.zshrc; then
    echo '# ASDF Setup' >>~/.zshrc
    echo '. "$HOME/.asdf/asdf.sh"' >>~/.zshrc
  fi
  if ! grep -q ". \"\$HOME/.asdf/completions/asdf.bash\"" ~/.zshrc; then
    echo '. "$HOME/.asdf/completions/asdf.bash"' >>~/.zshrc
  fi

  # Source zshrc in the current script so asdf command is available immediately
  # WARNING: This might cause issues in some script execution environments.
  # A safer approach might be to instruct the user to restart their shell.
...
# A more precise check would be 'asdf list neovim | grep -q latest' but this is simpler.
if command_exists asdf; then
  if ! command_exists nvim; then
    ...
    # Check if 'latest' is already installed via asdf
    LATEST_NEOVIM=$(asdf latest neovim)
    if asdf list neovim | grep -q "$LATEST_NEOVIM"; then
  ...
  for package in "${STOW_PACKAGES[@]}"; do
    if [ -d "./$package" ]; then       # Check if the package directory exists in dotfiles
      ...
      stow "$package" --no-folding || {
        echo "Error: Failed to stow '$package'. Check warnings above or manual intervention may be needed."
        exit 1
      }
    else
